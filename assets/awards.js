/* Public standings are published only when config.reveal is enabled. */
(function(global){
 'use strict';
 ...
 const trophyPath=a=>a.trophy_image||`assets/awards/trophy-${a.trophy}.png`;
 ...
 function receipts(a,winner,standings){
  ...
  const buildEvidence=b=>`<tr><td>${esc(buildName(b))}</td><td>${fmt(b.total)} total stats · ${fmt(b.stats?.guts)} Guts · ${fmt(b.stats?.wisdom)} Wit · ${fmt(b.sp)} worth of skills (undiscounted)<br><a href="${esc(base+encodeURI(b.source))}" target="_blank" rel="noopener">Source export ↗</a></td></tr>`;
  ...
  return `<details class="award-proof"><summary>See the receipts</summary><p>${winner.build?'One individual Uma build. Other builds and rounds do not add to this score.':`${fmt(p.starts)} races · ${fmt(p.build_count)} fielded builds · ${esc(p.rounds.join(', '))}`}</p>${winner.tiebreaks?.length?`<p>Tiebreakers: ${winner.tiebreaks.map(t=>esc(t.field.replaceAll('_',' '))+': '+(t.field==='tournament_champion'?(t.value?'Yes':'No'):fmt(t.value,4))).join(' · ')}</p>`:''}<div class="award-proof-scroll"><table><thead><tr><th>Record</th><th>Evidence</th></tr></thead><tbody>${rows}</tbody></table></div>${a.metric==='efficiency'?`<p>${fmt(p.points)} points ÷ ${fmt(p.base_total)} stats × 1,000 = ${fmt(p.efficiency,6)}</p>`:''}</details>`;
 }
 function awardCard(a,options={}){
  const revealed=Boolean(options.revealed),winner=revealed?a.winner:null;
  return `<article class="award-card award-${esc(a.id)}" data-award="${esc(a.id)}"><div class="award-art"><img src="${esc(asset(a.image,options))}" alt="${esc(a.name)} artwork"><span class="award-category">${esc(a.category)}</span></div><div class="award-copy"><h2 id="award-detail-title">${esc(a.name)}</h2><p class="award-description">${esc(a.description)}</p><div class="award-winner"><img class="award-trophy${a.trophy_image?' trophy-dark':''}" src="${esc(asset(trophyPath(a),options))}" alt=""><div class="award-name"><span>${revealed?(winner?(options.local?'CURRENT LEADER':'WINNER'):a.status==='tie'?'TIED AT THE TOP':'AWAITING EVIDENCE'):'WINNER'}</span><strong>${winner?esc(winner.name):revealed?(a.status==='tie'?a.runners_up.map(p=>esc(p.name)).join(' & '):'Pending'):'To be revealed'}</strong><small>${winner?esc(winner.team)+(winner.build?'<br>'+esc(buildName(winner.build)):''):revealed?esc(a.reason||'No qualifying record yet.'):'Revealed after the tournament.'}</small></div><div class="award-number"><b>${winner?value(winner,a):revealed&&a.status==='tie'?value(a.runners_up[0],a):'—'}</b><small>${esc(a.unit)}</small></div></div><details class="award-runners"><summary>${a.status==='tie'&&revealed?'Tied contenders':'Runners-up'} <span>${revealed?'Final standings':'Revealed at the final'}</span></summary><ol>${revealed?(a.runners_up?.length?a.runners_up.map((p,i)=>`<li><span>${a.status==='tie'?'=':String(i+2).padStart(2,'0')}</span><div><strong>${esc(p.name)}</strong><small>${esc(p.team)}${p.build?'<br>'+esc(buildName(p.build)):''}</small></div><b>${value(p,a)} <small>${esc(a.unit)}</small></b></li>`).join(''):'<li class="award-no-runner">No other qualifying contender yet.</li>'):'<li class="award-no-runner">Names and values will appear at the final reveal.</li>'}</ol>${revealed&&a.tie_count>2?`<p class="tie-note">${a.tie_count} contenders share the top value. Showing two. Equal scores remain tied under the published rules.</p>`:''}</details><details class="award-proof award-rule"><summary>How this award is decided</summary><p>${esc(a.rule)}</p></details>${revealed?receipts(a,winner,options.standings):''}</div></article>`;
 }
 function trophy(a,options){
  const winner=options.revealed?a.winner:null;
  return `<button type="button" class="trophy-slot" data-open-award="${esc(a.id)}" aria-haspopup="dialog" aria-label="View ${esc(a.name)}"><span class="trophy-stage"><span class="trophy-light"></span><img class="shelf-trophy${a.trophy_image?' trophy-dark':''}" src="${esc(asset(trophyPath(a),options))}" alt="" loading="lazy"><span class="shelf-preview" aria-hidden="true"><img src="${esc(asset(a.image,options))}" alt="" loading="lazy"></span></span><span class="trophy-plaque"><span class="plaque-name">${esc(a.name)}</span><span class="plaque-status">${options.revealed?(winner?esc(winner.name)+' · '+value(winner,a):a.status==='tie'?'Tie · '+a.runners_up.map(p=>esc(p.name)).join(' & '):'Pending evidence'):'View award'} <span aria-hidden="true">↗</span></span></span></button>`;
 }
 ...
 function bindCase(root,cards,options={}){
  ...
  if(options.local)for(const link of root.querySelectorAll('.case-nav a, .meta-nav a'))link.onclick=e=>{e.preventDefault();root.querySelector(link.getAttribute('href'))?.scrollIntoView({behavior:'smooth',block:'start'});};
 ...
 function playerTable(standings,query='',view='results'){
  const columns={results:[['starts','Races'],['points','Points'],['firsts','1st'],['seconds','2nd'],['thirds','3rd']],builds:[['build_count','Builds'],['single_stats','Highest stats / Uma'],['single_guts','Highest Guts / Uma'],['single_wisdom','Lowest Wit / Uma'],['single_sp','Highest skill value / Uma']],events:[['zero_hp_finishes','0 HP finishes'],['late_starts','Late starts'],['rushed','Rushed'],['failed_wit','Wit failures'],['blocked_incidents','Blocked count'],['blocked_seconds','Blocked seconds'],['duel_seconds','Duel seconds ≈'],['lane_loss_m','WT metres lost ≈']]}[view];
 ...
 async function publicMount(){
  ...
  try{
   const [index,config,snapshot]=await Promise.all([getJSON('data/tournament-index.json'),getJSON('config/awards.json'),getJSON('data/tournament-statistics.json')]);
   if(snapshot.revision!==index.revision)throw Error('Statistics are updating. Please refresh in a moment.');
   const standings=config.reveal?snapshot.standings:null,options={revealed:Boolean(config.reveal&&standings),standings,statistics:snapshot.statistics};
   root.innerHTML=page(index,config,standings,options);bindCase(root,options.revealed?standings.awards:AwardEngine.catalog(config),options);
  }
 ...
 function previewMount(seed){
  let {index,config,standings,skills,assets,telemetryData,statistics}=seed,revealed=true;
  const root=document.getElementById('awards-root');
  function render(){
   root.innerHTML=page(index,config,standings,{local:true,revealed,assets,statistics});
  ...
  async function refresh(){
   ...
   try{
    const [nextIndex,nextSkills,nextSnapshot]=await Promise.all([getJSON(base+'data/tournament-index.json'),getJSON(base+'assets/award-skills.json'),getJSON(base+'data/tournament-statistics.json')]);statistics=nextSnapshot.statistics;
