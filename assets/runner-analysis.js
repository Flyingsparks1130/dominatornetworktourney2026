...
(function(global){
 ...
 function open({data,runner:r,labels,teamName,sample,seek}){
  ...
  const rank=Number(r.training_rank),high=['UG','UF','UE','UD','UC','UB','UA'];
  const rankLabel=rank>=19?((high[Math.floor((rank-19)/10)]||'U')+((rank-19)%10||'')):(grades[rank]||'—');
  dialog.querySelector('.ra-style').insertAdjacentHTML('afterend',`<div class="ra-training"><span>Training evaluation<strong>${Number.isFinite(r.training_score)?r.training_score.toLocaleString('en-US'):'—'}</strong></span><b class="ra-rank">${esc(rankLabel)}</b></div>`);
  dialog.querySelector('.ra-aptitudes').innerHTML=[['Ground',['turf','dirt']],['Distance',['sprint','mile','medium','long']],['Style',['front','pace','late','end']]].map(([label,keys])=>`<div class="ra-aptitude-row"><small>${label}</small>${keys.map(key=>`<span>${key}<b data-grade="${esc(r.aptitudes?.[key])}">${esc(r.aptitudes?.[key]??'—')}</b></span>`).join('')}</div>`).join('');
  dialog.querySelector('.ra-supports').outerHTML=`<section class="ra-supports"><h3>Support cards</h3><div class="ra-support-grid">${(r.support||[]).map(s=>`<figure><img src="https://gametora.com/images/umamusume/supports/support_card_s_${encodeURIComponent(s.id)}.png" alt="Support card ${esc(s.id)}" loading="lazy"><figcaption aria-label="${esc(s.limit_breaks??'Unknown')} limit breaks">${Number.isInteger(s.limit_breaks)&&s.limit_breaks>=0&&s.limit_breaks<=4?'◆'.repeat(s.limit_breaks)+'◇'.repeat(4-s.limit_breaks):'—'}<small>${esc(s.limit_breaks??'—')}/4 LB</small></figcaption></figure>`).join('')||'<p>No support cards recorded.</p>'}</div></section>`;
